...
//      cout<<WZTree->l_pt1<<"   pt1"<<endl;
 ...
 //    cout<<"DEBUG: 4:" << endl;
    ...
    //  cout<<WZTree->l_pt3<<"   pt3"<<endl;
      ...
      WZTree->trilep_m = (tightEle[tightEle_ZL1_index]+tightEle[tightEle_ZL2_index]+tightMuon[tightMu_WL_index]).M();
      // //preselection on met
...
cutEff[11]++;  												//loop 3 ends
 ...
 //     cout<<"DEBUG: 3:" << endl;
  ...
  std::cout<<"(0) all events:        "<<cutEff[0]<<"\t:\t"<<((float)cutEff[0]*100.0)/(float)cutEff[0]<<std::endl
	  ...
	  <<"(2) Exactly 2 electron:  "<<cutEff[2]<<"\t:\t"<<((float)cutEff[2]*100.0)/(float)cutEff[0]<<std::endl
	  <<"(3) electron filled correctly:      "<<cutEff[3]<<"\t:\t"<<((float)cutEff[3]*100.0)/(float)cutEff[2]<<std::endl
	  <<"(4) 3rd lepton muon:               "<<cutEff[4]<<"\t:\t"<<((float)cutEff[4]*100.0)/(float)cutEff[3]<<std::endl
	  <<"(5) Dilepto-Zmass:               "<<cutEff[5]<<"\t:\t"<<((float)cutEff[5]*100.0)/(float)cutEff[4]<<std::endl
