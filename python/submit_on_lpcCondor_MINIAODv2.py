...
if TestRun:
	outputFolder = "/store/user/gchaudha/WZTree_"+datetime.datetime.now().strftime('%Y_%m_%d_%Hh%M')+"_TEST/";
	OutputLogPath = "OutPut_Logs/Logs_" + datetime.datetime.now().strftime('%Y_%m_%d_%Hh%M') + "_TEST";
else:
	outputFolder = "/store/user/gchaudha/WZTree_"+datetime.datetime.now().strftime('%Y_%m_%d_%Hh%M');
	OutputLogPath = "OutPut_Logs/Logs_" + datetime.datetime.now().strftime('%Y_%m_%d_%Hh%M');

print "Name of output dir: ",outputFolder
# prints name of output folder
...
# Get CMSSW directory path and name & os.environ Get the users environment 
...
#it will write a script runstep2condor.sh in which commands below will be executed
outScript = open("runstep2condor.sh","w");
#The first argument is a string containing the filename. The second argument is another string containing a few characters describing the way in which the file will be used. w is for write purpose
...
outScript.write("\n");
outScript.close();
#There will be a Permission tab where you can change the file permissions. chmod 777 means making the file readable, writable and executable by everyone.
os.system("chmod 777 runstep2condor.sh");

#this is the script used for condor job submission & it has al the commands described below
...
# ERROR is the error channel from your job that HTCondor returns from the remote host.
...
if( doMC ):
    for i in range(len(samples)):
      ...
      ##
      #######################################################################
      print("xrdfs root://cmseos.fnal.gov ls "+inputFolder+str(samples[i][1])+str(samples[i][4])+" | awk '{print \"root://cmseos.fnal.gov/\"$1}' > temp.txt")
      print "\n\n"
      os.system("xrdfs root://cmseos.fnal.gov ls "+inputFolder+str(samples[i][1])+str(samples[i][4])+" | awk '{print \"root://cmseos.fnal.gov/\"$1}' > temp.txt")
